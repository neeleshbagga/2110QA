...
public class QuestionOverviewController {
	...
	public void submitPressed() {
		...
		if (selection == null) {
			Dialogs.create()
			.title("Error:")
			.masthead("Make a selection, and then submit.")
			.message(
					"")
			.showWarning();
	...
	public void explainPressed() {
		if (submitPressed) {
			...
			Dialogs.create()
					.title("")
					.masthead("Question needs to be attempted.")
					.message(
							"Sorry, please answer the question and then try to view the explanation.")
					.showWarning();
		}
	}
	...
	private void displayNextQuestion() {
		...
		submitPressed = false;
		count++;
		progressBar.setProgress((double) count
				/ topicLength.get(currQ.getTopic().getTopicName()));
		...
		if (currentQuestions.hasNext()) {
			...
			Dialogs.create()
					...
					.message(
							"Kudos. You have completed all the questions in this topic. Please select another topic to proceed")
					.showWarning();
		}
	}
	...
	private void showTopicQuestions(Topic topic) {
		...
		currentQuestions = QAUtils.filterQuestionsByTopic(allQuestions, topic)
				.iterator();
